...
def process_data(files, save_every_n_files, model_info=-1, normalize=True, model_dir=None):
    ...
    total_events_raw = 0 #before any gen-pt or geometry filter
    total_wafers_raw = 0 #before geometry filter
    total_wafers_geom = 0 #after geometry/eLink mas
    ...
    for i, file in enumerate(files):
        ...
        total_wafers_raw += len(combined_df)
        ...
        total_wafers_geom += len(filtered_df)
    ...
    print("\n========== DATASET SUMMARY ==========")
    print(f" ROOT files scanned       : {len(files)}")
    print(f" Raw events               : {total_events_raw:,}")
    print(f" Raw wafers (all layers)  : {total_wafers_raw:,}")
    print(f" Wafers after geom mask   : {total_wafers_geom:,}")
    print("======================================\n")

    summary = {
        "timestamp"          : datetime.utcnow().isoformat(timespec="seconds") + "Z",
        "host"               : socket.gethostname(),
        "model_mode"         : "eLink" if args.model_per_eLink else "bits",
        "model_value"        : model_info,               # e.g. 5  (eLinks) or 7 (bits)
        "alloc_geom"         : args.alloc_geom,          # old / new
        "n_root_files"       : len(files),
        "raw_events"         : int(total_events_raw),
        "raw_wafers"         : int(total_wafers_raw),
        "geomsel_wafers"     : int(total_wafers_geom)
    }

    # 2.  Write a one‑off JSON into the current model directory
    json_path = os.path.join(model_dir, "dataset_summary.json")
    with open(json_path, "w") as jf:
        json.dump(summary, jf, indent=2)
    print(f"Summary written to {json_path}")

    # 3.  Append (or create) a central CSV log under --opath
    csv_path = os.path.join(args.opath, "dataset_log.csv")
